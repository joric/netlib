...
static inline unsigned int rol(unsigned int v, int s)
{
	return (v << s) | (v >> (32 - s));
}

static inline void swap_words(unsigned int * w, int n)
{
#ifdef WORDS_BIGENDIAN
	for (; n > 0; w++, n--)
	{
		unsigned int v = rol(*w, 16);
		*w = ((v >> 8) & 0x00FF00FF) | ((v << 8) & 0xFF00FF00);
	}
#endif
}

static inline void put_word(unsigned char * dst, unsigned int val)
{
#ifdef WORDS_BIGENDIAN
	dst[0] = val;
	dst[1] = val >> 8;
...
static void md5_mix(struct md5_ctx *ctx, const unsigned int * X)
{
	unsigned int a, b, c, d;
...
void md5_final(unsigned char * dst, struct md5_ctx *ctx)
{
	static const unsigned char padding[MD5_BLOCK_LENGTH] = { 0x80 };
	unsigned long long final_len = ctx->nbytes * 8;
